...
#
# v2 (under_my_spell): the STEREO PHASE CORRELATION block was
# rewritten. The previous grep-based approach produced empty
# output (the aphasemeter running log was not captured). It now
# aggregates per-frame lavfi.aphasemeter.phase metadata into
# mean / min / max, which is the correlation decision input for
# whether stereo widening is safe. All other features unchanged.
# ============================================================
set -e
...
# --- Stereo phase correlation (mean / min / max) --- [v2: working aggregation]
echo ""
echo "=== STEREO PHASE CORRELATION ==="
ffmpeg -hide_banner -nostats -i "$SOURCE" \
    -af "aphasemeter=video=0,ametadata=print:key=lavfi.aphasemeter.phase:file=-" \
    -f null - 2>/dev/null | grep "lavfi.aphasemeter.phase" | awk -F'=' '
    { v=$2; sum+=v; n++; if(n==1){min=v;max=v} if(v<min)min=v; if(v>max)max=v }
    END{ if(n>0) printf "  frames: %d\n  mean:   %+.3f\n  min:    %+.3f\n  max:    %+.3f\n", n, sum/n, min, max;
         else print "  (no phase data - mono source?)" }'
