...
# ------------------------------------------------------------
# DEFAULT on WSL2 (fast, true-peak compliant, deterministic):
#   E0: FFmpeg asoftclip  (gentle soft-clip with 4x oversample;
#       catches peaks before the hard limiter; ~5 sec for 2:22)
#   E1: alimiter inside a 4x soxr-oversampled scaffold (brickwall
#       true-peak). Verified COMPLIANT across input drive levels.
#
# LEGACY LV2 paths (slow on WSL2; lv2apply processes 1 sample/syscall):
#   E0_CLIPPER=cliponly2  -> Airwindows ClipOnly2 (hangs on this WSL2)
#   E0_CLIPPER=lsp        -> LSP clipper (works but ~20 min for 2:22)
#
...
# WSL2: default to ffmpeg (fast). Override with E0_CLIPPER=cliponly2 or lsp.
E0_CLIPPER="${E0_CLIPPER:-ffmpeg}"
...
# ---- E0: soft-clip stage ----
if [ "$E0_CLIPPER" = "ffmpeg" ]; then
  # Fast native FFmpeg soft-clip: threshold slightly below alimiter ceiling
  # so the clipper does gentle rounding before the brickwall limiter.
  TH_CLIP_LIN=$(db2lin "$(python3 -c "print($TP_DBTP + $CLIP_HEADROOM_DB)")")
  ffmpeg -hide_banner -nostats -y -i "$IN" \
    -af "asoftclip=threshold=$TH_CLIP_LIN:param=1.0:oversample=4" \
    -c:a pcm_f32le "$TMP/e0.wav" 2>/dev/null

elif [ "$E0_CLIPPER" = "lsp" ]; then
  ...
  # Try ClipOnly2 with a 30s timeout; fallback to LSP clipper if it hangs (WSL2 quirk)
  if timeout 30 lv2apply -i "$IN" -o "$TMP/e0raw.wav" "$CLIPONLY2_URI" >/dev/null 2>&1; then
    ffmpeg -hide_banner -nostats -y -i "$TMP/e0raw.wav" -af "atrim=start_sample=1,asetpts=N/SR/TB" "$TMP/e0.wav" 2>/dev/null
  else
    echo "[stage_clip_limit] WARNING: ClipOnly2/lv2apply hung or failed. Falling back to LSP clipper."
    TH_CLIP=$(db2lin "$(python3 -c "print($TP_DBTP + $CLIP_HEADROOM_DB)")")
    lv2apply -i "$IN" -o "$TMP/e0.wav" -c ce 1 -c thresh "$TH_CLIP" -c ct 0.92 -c dither 0 "$LSP_CLIP_URI" 2>/dev/null
  fi
fi

# ---- E1: true-peak limit stage ----
