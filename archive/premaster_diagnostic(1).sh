...
#
# v2 FIX (restored every session): the v1 phase-correlation block
# grepped `aphasemeter=video=0` stdout, which prints nothing parseable.
# v2 dumps per-frame `lavfi.aphasemeter.phase` metadata via ametadata
# and aggregates it into mean/min/max + frame count with awk.
#
...
ffmpeg -hide_banner -nostats -i "$SOURCE" \
    ...
    tail -14 | grep -E "I:|LRA:|Peak:|range:|Threshold:|low:|high:"
...
ffmpeg -hide_banner -nostats -i "$SOURCE" \
    ...
    -f null - 2>&1 | grep -E "DC offset|Peak level dB|RMS level dB|Crest factor" | head -8

# --- Stereo phase correlation (per-frame aggregation) [v2 FIX] ---
echo ""
...
ffmpeg -hide_banner -nostats -i "$SOURCE" \
    ...
    awk -F= '/lavfi.aphasemeter.phase/ {
        ...
        if (n>0) {
            ...
            printf "  frames analysed  : %d\n", n
        } else {
            print "  (no phase frames captured)"
        }
    }'
